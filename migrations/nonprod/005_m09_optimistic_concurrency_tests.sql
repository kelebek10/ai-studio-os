-- PAI-FORGE M09 OPTIMISTIC CONCURRENCY NONPROD TEST v1.2
...
DO $$
DECLARE
 v_design uuid:=gen_random_uuid(); v_ds1 uuid; v_ds2 uuid; v_cr uuid; v_cr_stale uuid;
 v_actor uuid:=gen_random_uuid(); v_other uuid:=gen_random_uuid(); v_result text; v_event1 uuid; v_event2 uuid;
BEGIN
 INSERT INTO governance.design_state(design_id,state_version,state_hash,ruleset_version,created_by) VALUES(v_design,1,'m09-hash-v1','m09-r1',v_actor) RETURNING design_state_id INTO v_ds1;
 INSERT INTO governance.design_state(design_id,state_version,state_hash,ruleset_version,created_by) VALUES(v_design,2,'m09-hash-v2','m09-r1',v_actor) RETURNING design_state_id INTO v_ds2;
 INSERT INTO governance.change_request(request_key,design_state_id,expected_state_version,canonical_request_hash,status,created_by) VALUES('m09-cr-001',v_ds1,1,'m09-request-001','CANDIDATE',v_actor) RETURNING change_request_id INTO v_cr;
 INSERT INTO governance.change_request(request_key,design_state_id,expected_state_version,canonical_request_hash,status,created_by) VALUES('m09-cr-stale',v_ds2,2,'m09-request-stale','CANDIDATE',v_actor) RETURNING change_request_id INTO v_cr_stale;
 ...
 -- Replay must be checked before predecessor/version validation in the transition function.
 BEGIN
 ...
 EXCEPTION WHEN OTHERS THEN
   INSERT INTO m09_results VALUES('M09-N04','BLOCKED','Transition function validates predecessor before idempotent replay lookup: '||SQLERRM);
 END;

 IF to_regclass('governance.design_head') IS NULL AND to_regprocedure('governance.compare_and_set_design_head(uuid,bigint,text)') IS NULL THEN
