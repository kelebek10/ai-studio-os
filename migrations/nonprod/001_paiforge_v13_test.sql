...
CREATE OR REPLACE FUNCTION governance.validate_constraint_version() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE v_classification text;
BEGIN
 SELECT classification INTO v_classification FROM core.constraint WHERE constraint_id=NEW.constraint_id;
 IF v_classification IS NULL THEN RAISE EXCEPTION 'CONSTRAINT_ID_NOT_FOUND'; END IF;
 IF v_classification='SAFETY_ENGINEERING' AND NEW.enforcement<>'NON_NEGOTIABLE' THEN
   RAISE EXCEPTION 'SAFETY_ENGINEERING_REQUIRES_NON_NEGOTIABLE';
 END IF;
 RETURN NEW;
END; $$;
CREATE TRIGGER constraint_version_policy BEFORE INSERT OR UPDATE ON core.constraint_version FOR EACH ROW EXECUTE FUNCTION governance.validate_constraint_version();
