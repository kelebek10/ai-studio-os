-- M18 non-production only: persistent bounded Agent Registry authority boundary.
-- Authority is enforced by PostgreSQL role + RLS; no caller-supplied actor metadata is trusted.
DROP SCHEMA IF EXISTS m18_agent_registry CASCADE;
CREATE SCHEMA m18_agent_registry;
...
CREATE TABLE m18_agent_registry.agent (
  agent_id text PRIMARY KEY,
  name text NOT NULL,
  tier text NOT NULL CHECK (tier IN ('CORE','SPECIALIST')),
  scope text NOT NULL,
  provider text,
  parent_agent_id text REFERENCES m18_agent_registry.agent(agent_id),
  status text NOT NULL DEFAULT 'ACTIVE' CHECK (status IN ('ACTIVE','DISABLED')),
  approval_id text,
  created_by_principal text NOT NULL DEFAULT current_user,
  created_at timestamptz NOT NULL DEFAULT now(),
  CHECK ((tier='CORE' AND provider IS NULL AND parent_agent_id IS NULL AND approval_id IS NOT NULL AND approval_id <> '')
      OR (tier='SPECIALIST' AND provider IN ('GEMINI','CLAUDE','COPILOT','QWEN') AND parent_agent_id IS NOT NULL AND approval_id IS NULL))
);

CREATE TABLE m18_agent_registry.audit_event (
...
CREATE TRIGGER agent_enforce_limits BEFORE INSERT ON m18_agent_registry.agent
FOR EACH ROW EXECUTE FUNCTION m18_agent_registry.enforce_limits();
CREATE TRIGGER agent_audit_insert AFTER INSERT ON m18_agent_registry.agent
FOR EACH ROW EXECUTE FUNCTION m18_agent_registry.audit_agent_insert();
...
CREATE POLICY agent_insert_orchestrator ON m18_agent_registry.agent FOR INSERT TO paiforge_m18_orchestrator
  WITH CHECK (tier='SPECIALIST' AND provider IN ('GEMINI','CLAUDE','COPILOT','QWEN') AND approval_id IS NULL);
CREATE POLICY agent_insert_human ON m18_agent_registry.agent FOR INSERT TO paiforge_m18_human_approver
  WITH CHECK (tier='CORE' AND provider IS NULL AND parent_agent_id IS NULL AND approval_id IS NOT NULL AND approval_id <> '');
...
CREATE POLICY audit_insert_orchestrator ON m18_agent_registry.audit_event FOR INSERT TO paiforge_m18_orchestrator WITH CHECK (actor_principal=current_user);
CREATE POLICY audit_insert_human ON m18_agent_registry.audit_event FOR INSERT TO paiforge_m18_human_approver WITH CHECK (actor_principal=current_user);
...
GRANT SELECT,INSERT ON m18_agent_registry.audit_event TO paiforge_m18_orchestrator,paiforge_m18_human_approver;
